...
def local_dossier(project:Project,brief:dict[str,str],source_names:list[str])->dict[str,object]:
    ...
    def add_gap(vacio:str,por_que_importa:str,pregunta:str,aproximacion:str,evidencia_ideal:str,brief_key:str):
        ...
        gaps.append({"vacio":vacio,"estado":"Base cargada · contrastar por aproximación" if answer else "Aproximación pendiente","respuesta_cargada":answer[:900],"por_que_importa":por_que_importa,"pregunta":pregunta,"aproximacion_suficiente":aproximacion,"evidencia_ideal":evidencia_ideal,"evidencia_necesaria":aproximacion})
    ...
    routes=[
        {"nombre":"Reencuadrar la categoría","hipotesis":"Construir relevancia desde una tensión humana todavía por validar.","condicion_para_elegirla":gaps[0]["vacio"] if gaps else insufficient},
    ...
    base.update({
        ...
        "fuentes_y_calidad":{"fuentes":source_names or ["Brief inicial"],"evaluacion":"Las fuentes aportan contexto de categoría, pero no sustituyen evidencia propia de consumidores, canales, producto y operación."},
        ...
        "que_no_sabemos":gaps,
        "diagnostico_del_problema":{"hallazgo_preliminar":"El desafío parece ser construir una propuesta elegible y escalable, no solamente lanzar una marca.","hipotesis_a_refutar":"Si la gente conoce la nueva marca, la comprará al precio y en el canal propuestos.","por_que_no_es_conclusion":f"Antes de cerrar una ruta, conviene contrastar por aproximación: {', '.join(top_names)}."},
        ...
        "comportamiento_a_cambiar":brief.get("behavior") or {"pendiente":"Definir una sola conducta observable del segmento prioritario.","pregunta":gaps[0]["pregunta"] if gaps else insufficient},
        ...
        "proxima_decision":{"decision_posible_ahora":"Se puede elegir una ruta de trabajo y avanzar con aproximaciones explícitas.","no_cerrar_aun":"No presentar las aproximaciones como certezas universales; revisarlas con ventas, reposición y aprendizaje del lanzamiento.","resolver_primero":top_names,"plan_de_validacion":[gap["aproximacion_suficiente"] for gap in gaps[:3]],"criterio_de_salida":"Cuando estas aproximaciones estén documentadas, generar una nueva versión y elegir una ruta de lanzamiento bajo revisión."},
    })
    return base
