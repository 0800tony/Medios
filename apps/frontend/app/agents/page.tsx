...
export default function AgentsPage() {
  ...
  async function run(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!projectId) return setError("Elegí un proyecto"); const form = event.currentTarget; setBusy(true); setError(""); try { const data = new FormData(form); await request(`/api/projects/${projectId}/agents/run`, { method: "POST", body: JSON.stringify({ agent_key: selected, instruction: data.get("instruction") }) }); form.reset(); setRuns(await request<AgentRun[]>(`/api/projects/${projectId}/agents`)); } catch (error) { setError((error as Error).message); } finally { setBusy(false); } }
