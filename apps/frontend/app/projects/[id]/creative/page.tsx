...
export default function Creative({ params }: { params: { id: string } }) {
  ...
  return <main className="shell"><Nav/>
    ...
    {selectedPlan?.status === "approved" && planBoard && productionItems(planBoard).length > 0 ? <section className="card production-desk"><div className="campaign-plan-head"><div><p className="eyebrow">Dirección creativa y producción</p><h2>La idea primero. Los guiones, después.</h2><p className="muted">Cada pieza parte de una tensión, un giro y una frase rectora. Abrí sólo el guion que querés trabajar.</p></div><span className="status">listo para producir</span></div>
      ...
      <div id="bocetos" className="visual-board"><div className="visual-head"><div><p className="eyebrow">Estudio Creativo Visual</p><h3>Primero, el boceto maestro de la idea aprobada</h3><p className="muted">No tenés que encargar ni describir la imagen: OLIVA toma la plataforma creativa aprobada, su tensión, la idea rectora, identidad y territorio para visualizar el concepto que el equipo debe desarrollar.</p></div></div><div className="visual-request"><div><strong>Boceto maestro de campaña</strong><p className="muted">Una previsualización de la idea general. Después podés generar aplicaciones específicas de las direcciones que recomienda el plan.</p></div><button className="btn lime" disabled={busy} onClick={() => generateVisual()}>{busy ? "Generando boceto…" : "Generar boceto maestro →"}</button>{error && <p className="error visual-error" role="alert">{error}</p>}</div><div className="visual-grid">{visuals.map(item => <article key={item.id} className="visual-draft">{visualUrls[item.id] ? <img src={visualUrls[item.id]} alt={`Boceto generado: ${item.title}`} /> : <div className="visual-prompt"><span>{item.status}</span><strong>{item.title}</strong></div>}<div><span className="status">imagen generada por OLIVA</span><h4>{item.title}</h4><details><summary>Ver dirección usada</summary><p>{item.prompt}</p></details></div></article>)}{visualDrafts(planBoard).map(item => <article key={item.id} className="visual-draft"><div className="visual-prompt"><span>Aplicación recomendada</span><strong>{item.pieza}</strong></div><div><span className="status">{item.estado}</span><h4>{item.titulo}</h4><p>{item.direccion}</p><button type="button" className="text-action" disabled={busy} onClick={() => generateVisual(String(item.titulo || "Boceto de campaña"), String(item.prompt_de_produccion || item.direccion || ""))}>Generar aplicación →</button></div></article>)}</div></div>
