...
export default function Creative({ params }: { params: { id: string } }) {
  ...
  async function savePlan(status: "draft" | "approved") {
    ...
    try {
      ...
      setPlans(current => [updated, ...current.filter(item => item.id !== updated.id)]); setSaved(status === "approved" ? "Plan de campaña aprobado. Ya podés producir y cargar cada material para revisión." : "Plan de campaña guardado. Podés corregir piezas, soportes y cobertura antes de aprobarlo."); await load();
    } catch (error) { setError((error as Error).message); } finally { setBusy(false); }
  }
