...
export default function Creative({ params }: { params: { id: string } }) {
  ...
  async function generateVisual(title?: string, focus?: string) {
    if (!selectedPlan) return; setBusy(true); setError(""); setSaved("");
    try { const body = title || focus ? { title: title || "Boceto de campaña", focus: focus || "" } : {}; const draft = await request<CreativeVisual>(`/api/projects/${params.id}/creative-plans/${selectedPlan.id}/visuals/generate`, { method: "POST", body: JSON.stringify(body) }); setVisuals(current => [draft, ...current]); setSaved(title || focus ? "Boceto de aplicación generado desde la decisión elegida." : "Boceto maestro generado desde la plataforma aprobada y guardado en el expediente creativo."); }
    catch (error) { setError((error as Error).message); } finally { setBusy(false); }
  }
  async function archiveVisual(item: CreativeVisual) {
    setBusy(true); setError("");
    try { await request<CreativeVisual>(`/api/projects/${params.id}/creative-visuals/${item.id}`, { method: "PATCH", body: JSON.stringify({ status: "archived" }) }); setVisuals(current => current.filter(visual => visual.id !== item.id)); setSaved("Boceto retirado del tablero. Queda preservado en el expediente, sin afectar la estrategia ni el plan."); }
    catch (error) { setError((error as Error).message); } finally { setBusy(false); }
  }
  ...
  return <main className="shell"><Nav/>
    <div className="pagehead"><div><p className="eyebrow">Dirección creativa</p><h1>{project.name}</h1></div><div className="page-actions"><Link className="btn ghost" href={`/projects/${project.id}/result`}>← Estrategia</Link><Link className="btn ghost" href={`/projects/${project.id}`}>Proyecto</Link></div></div><ProjectFlow projectId={project.id} current="creative" hasStrategy={ready}/>
    ...
    {selectedPlan?.status === "approved" && planBoard && productionItems(planBoard).length > 0 ? <section className="card production-desk"><div className="campaign-plan-head"><div><p className="eyebrow">Dirección creativa y producción</p><h2>La idea primero. Los guiones, después.</h2><p className="muted">Cada pieza parte de una tensión, un giro y una frase rectora. Abrí sólo el guion que querés trabajar.</p></div><span className="status">listo para producir</span></div>
      ...
      <div id="bocetos" className="visual-board"><div className="visual-head"><div><p className="eyebrow">Estudio Creativo Visual</p><h3>El boceto maestro nace de la idea aprobada</h3><p className="muted">La escena es protagonista; OLIVA aplica su identidad real alrededor de la imagen. No se generan textos, logos, mapas ni composiciones decorativas dentro del boceto.</p></div></div><div className="visual-request"><div><strong>Boceto maestro de campaña</strong><p className="muted">Una previsualización para el equipo. Las aplicaciones específicas se derivan después de esta idea.</p></div><button className="btn lime" disabled={busy} onClick={() => generateVisual()}>{busy ? "Generando boceto…" : "Generar boceto maestro →"}</button>{error && <p className="error visual-error" role="alert">{error}</p>}</div><div className="visual-grid">{visuals.map(item => <article key={item.id} className="visual-draft">{visualUrls[item.id] ? <img src={visualUrls[item.id]} alt={`Boceto generado: ${item.title}`} /> : <div className="visual-prompt"><span>{item.status}</span><strong>{item.title}</strong></div>}<div><span className="status">boceto de campaña · versión editable</span><h4>{item.title}</h4><p className="muted smallprint">Si esta versión no alcanza el nivel creativo esperado, retirala y generá una nueva desde la plataforma aprobada; nunca se modifica la estrategia.</p><div className="row-actions"><button className="text-action" disabled={busy} onClick={() => archiveVisual(item)}>Retirar del tablero</button><button className="text-action" disabled={busy} onClick={() => generateVisual(`Nueva versión — ${item.title}`, "Proponer una interpretación más propia, sobria y conceptualmente precisa de la misma plataforma; sin texto ni decoración.")}>Nueva versión →</button></div></div></article>)}{visualDrafts(planBoard).map(item => <article key={item.id} className="visual-draft"><div className="visual-prompt"><span>Aplicación recomendada</span><strong>{item.pieza}</strong></div><div><span className="status">{item.estado}</span><h4>{item.titulo}</h4><p>{item.direccion}</p><button type="button" className="text-action" disabled={busy} onClick={() => generateVisual(String(item.titulo || "Boceto de campaña"), String(item.prompt_de_produccion || item.direccion || ""))}>Generar aplicación →</button></div></article>)}</div></div>
