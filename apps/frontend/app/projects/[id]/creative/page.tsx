...
export default function Creative({ params }: { params: { id: string } }) {
  ...
  return <main className="shell"><Nav/>
    ...
    {selectedPlan?.status === "approved" && planBoard && productionItems(planBoard).length > 0 ? <section className="card production-desk"><div className="campaign-plan-head"><div><p className="eyebrow">Mesa de producción</p><h2>Propuestas de campaña y guiones de trabajo</h2><p className="muted">Parten de la plataforma y el plan aprobados. Son editables: corregí antes de encargar o producir materiales.</p></div><span className="status">listo para producir</span></div>
      ...
      <div className="visual-board"><div><p className="eyebrow">Bocetos visuales</p><h3>Dirección de arte antes de producir</h3><p className="muted">Son bocetos de criterio y composición: no reemplazan la aprobación de marca ni el arte final.</p></div><div className="visual-grid">{visualDrafts(planBoard).map(item => <article key={item.id} className="visual-draft">{item.asset_url ? <img src={item.asset_url} alt={`Boceto conceptual: ${item.titulo}`} /> : <div className="visual-prompt"><span>Boceto por generar</span><strong>{item.pieza}</strong></div>}<div><span className="status">{item.estado}</span><h4>{item.titulo}</h4><p>{item.direccion}</p><details><summary>Ver instrucción de producción</summary><p>{item.prompt_de_produccion}</p></details></div></article>)}</div></div>
