...
export default function Creative({ params }: { params: { id: string } }) {
  ...
  return <main className="shell"><Nav/>
    ...
    {selectedPlan?.status === "approved" && planBoard && productionItems(planBoard).length > 0 ? <section className="card production-desk"><div className="campaign-plan-head"><div><p className="eyebrow">Dirección creativa y producción</p><h2>La idea primero. Los guiones, después.</h2><p className="muted">Cada pieza parte de una tensión, un giro y una frase rectora. Abrí sólo el guion que querés trabajar.</p></div><span className="status">listo para producir</span></div>
      ...
      <div id="bocetos" className="visual-board"><div className="visual-head"><div><p className="eyebrow">Bocetos visuales</p><h3>Generar desde una decisión concreta</h3><p className="muted">Elegí una dirección o escribí un foco. Los bocetos se crean desde este plan aprobado; no se reutilizan imágenes de ejemplo.</p></div></div><div className="visual-request"><div className="field"><label>Qué querés visualizar</label><input value={visualFocus} onChange={event => setVisualFocus(event.target.value)} placeholder="Ej. pieza madre en un kiosco, activación de recreo, cartel de ruta…"/></div><button className="btn lime" disabled={busy} onClick={() => generateVisual()}>{busy ? "Generando…" : "Generar boceto solicitado →"}</button></div><div className="visual-grid">{visuals.map(item => <article key={item.id} className="visual-draft">{visualUrls[item.id] ? <img src={visualUrls[item.id]} alt={`Boceto generado: ${item.title}`} /> : <div className="visual-prompt"><span>{item.status}</span><strong>{item.title}</strong></div>}<div><span className="status">{item.model_used.includes("compositivo") ? "boceto generado desde el plan" : "generado por OLIVA"}</span><h4>{item.title}</h4><details><summary>Ver dirección usada</summary><p>{item.prompt}</p></details></div></article>)}{visualDrafts(planBoard).map(item => <article key={item.id} className="visual-draft"><div className="visual-prompt"><span>Dirección pendiente</span><strong>{item.pieza}</strong></div><div><span className="status">{item.estado}</span><h4>{item.titulo}</h4><p>{item.direccion}</p><button type="button" className="text-action" disabled={busy} onClick={() => generateVisual(String(item.titulo || "Boceto de campaña"), String(item.prompt_de_produccion || item.direccion || ""))}>Generar este boceto →</button></div></article>)}</div></div>
