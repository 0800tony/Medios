...
import { BrandAsset, CampaignPlanItem, CreativeConcept, CreativeProductionPlan, CreativeReview, CreativeTerritory, CreativeVisual, Dossier, Project, StrategyDecision, request, requestBlob } from "@/lib/api";
...
export default function Creative({ params }: { params: { id: string } }) {
  ...
  const load = async () => {
    ...
    const [assets, visualData] = await Promise.all([request<BrandAsset[]>(`/api/clients/${projectData.client_id}/brand-assets`), request<CreativeVisual[]>(`/api/projects/${params.id}/creative-visuals`)]);
    setProject(projectData); setConcepts(conceptData); setReviews(reviewData); setPlans(planData); setBrandAssets(assets); setVisuals(visualData);
  ...
  async function generateVisual() {
    ...
    try { const draft = await request<CreativeVisual>(`/api/projects/${params.id}/creative-plans/${selectedPlan.id}/visuals/generate`, { method: "POST", body: JSON.stringify({ title: "Boceto OLIVA de campaña", focus: "Desarrollar la pieza madre y sus aplicaciones de campaña con la identidad disponible." }) }); setVisuals(current => [draft, ...current]); setSaved("Boceto visual generado y guardado en el expediente creativo."); }
    catch (error) { setError((error as Error).message); } finally { setBusy(false); }
  }
  ...
  return <main className="shell"><Nav/>
    ...
    {selectedPlan?.status === "approved" && planBoard && productionItems(planBoard).length > 0 ? <section className="card production-desk"><div className="campaign-plan-head"><div><p className="eyebrow">Dirección creativa y producción</p><h2>La idea primero. Los guiones, después.</h2><p className="muted">Cada pieza parte de una tensión, un giro y una frase rectora. Abrí sólo el guion que querés trabajar.</p></div><span className="status">listo para producir</span></div>
      ...
      <nav className="creative-stage-nav" aria-label="Navegación de producción"><a href="#guiones">Guiones</a><a href="#mesa">Mesa OLIVA</a><a href="#bocetos">Bocetos</a><a href="#materiales">Materiales</a></nav>
      <div id="guiones" className="production-list">{productionItems(planBoard).map(item => <article className="production-item" key={item.id}><div className="production-item-head"><div><span className="status">{item.duracion_formato || item.medio || "formato"}</span><h3>{item.pieza || "Pieza de campaña"}</h3></div><p>{item.propuesta || "Definí la propuesta de ejecución."}</p></div><details><summary>Ver y editar guion de trabajo</summary><div className="production-item-editor"><div className="field two"><div><label>Pieza</label><input value={item.pieza || ""} onChange={event => updateProductionItem(item.id, "pieza", event.target.value)}/></div><div><label>Medio / formato</label><input value={item.duracion_formato || item.medio || ""} onChange={event => updateProductionItem(item.id, "duracion_formato", event.target.value)}/></div></div><div className="field"><label>Objetivo</label><textarea value={item.objetivo || ""} onChange={event => updateProductionItem(item.id, "objetivo", event.target.value)}/></div><div className="field"><label>Propuesta de ejecución</label><textarea value={item.propuesta || ""} onChange={event => updateProductionItem(item.id, "propuesta", event.target.value)}/></div><div className="field"><label>Guion técnico sugerido</label><textarea className="script-area" value={item.guion || ""} onChange={event => updateProductionItem(item.id, "guion", event.target.value)}/></div><div className="field"><label>Requisitos de producción</label><textarea value={item.produccion || ""} onChange={event => updateProductionItem(item.id, "produccion", event.target.value)}/></div><small><strong>Control final:</strong> {item.control}</small></div></details></article>)}</div>
      <div id="mesa" className="agent-board"><div><p className="eyebrow">Mesa OLIVA visible</p><h3>Qué controló cada agente</h3></div><div className="agent-grid">{agentItems(planBoard).map(item => <article key={item.agente}><span>{item.rol}</span><strong>{item.agente}</strong><p>{item.aporte}</p><small>{item.control}</small></article>)}</div></div>
      <div id="bocetos" className="visual-board"><div className="visual-head"><div><p className="eyebrow">Bocetos visuales</p><h3>Dirección de arte antes de producir</h3><p className="muted">Son bocetos de criterio y composición: no reemplazan la aprobación de marca ni el arte final.</p></div><button className="btn lime" disabled={busy} onClick={generateVisual}>{busy ? "Generando…" : "Generar boceto con fondo OLIVA →"}</button></div><div className="visual-grid">{visuals.map(item => <article key={item.id} className="visual-draft">{visualUrls[item.id] ? <img src={visualUrls[item.id]} alt={`Boceto generado: ${item.title}`} /> : <div className="visual-prompt"><span>{item.status}</span><strong>{item.title}</strong></div>}<div><span className="status">generado por OLIVA</span><h4>{item.title}</h4><details><summary>Ver dirección usada</summary><p>{item.prompt}</p></details></div></article>)}{visualDrafts(planBoard).map(item => <article key={item.id} className="visual-draft">{item.asset_url ? <img src={item.asset_url} alt={`Boceto conceptual: ${item.titulo}`} /> : <div className="visual-prompt"><span>Boceto por generar</span><strong>{item.pieza}</strong></div>}<div><span className="status">{item.estado}</span><h4>{item.titulo}</h4><p>{item.direccion}</p><details><summary>Ver instrucción de producción</summary><p>{item.prompt_de_produccion}</p></details></div></article>)}</div></div>
