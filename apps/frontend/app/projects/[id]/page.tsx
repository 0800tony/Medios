  ...
  return <main className="shell">
    ...
    <section className="project-layout">
      ...
      <aside className="card project-brief">
        {editingProject?<form onSubmit={updateProject}><p className="eyebrow">Editar expediente</p><div className="field"><label>Nombre</label><input name="name" required defaultValue={project.name}/></div><div className="field"><label>Etapa de trabajo</label><select name="workflow_stage" defaultValue={project.workflow_stage}><option value="ingreso">Ingreso</option><option value="informacion_incompleta">Información incompleta</option><option value="diagnostico">Diagnóstico</option><option value="estrategia">Estrategia</option><option value="ruta_seleccionada">Ruta seleccionada</option><option value="desarrollo_creativo">Desarrollo creativo</option><option value="medios">Planificación de medios</option><option value="produccion">Producción</option><option value="aprobacion">En aprobación</option><option value="ejecucion">En ejecución</option><option value="medicion">Medición</option><option value="cerrado">Cerrado</option></select></div><div className="field"><label>Empresa responsable</label><select name="group_company" defaultValue={project.group_company}><option>Oliva Publicidad</option><option>PUBLInter</option><option>Printshop Carteles</option><option>Veritas Auditor</option></select></div><div className="field"><label>Territorio</label><input name="territory" defaultValue={project.territory}/></div><div className="field"><label>Participantes</label><input name="participants" defaultValue={project.participants}/></div><div className="field"><label>Plazo</label><input name="deadline" defaultValue={project.deadline}/></div><div className="field"><label>Presupuesto conocido</label><input name="budget" defaultValue={project.budget}/></div><div className="field"><label>Objetivo declarado</label><input name="objective" defaultValue={project.objective}/></div><div className="field"><label>Brief / contexto</label><textarea name="brief" defaultValue={project.brief}/></div><div className="inline-actions"><button className="btn lime" disabled={busy}>Guardar cambios</button><button type="button" className="btn ghost" onClick={()=>setEditingProject(false)}>Cancelar</button></div></form>:<><p className="eyebrow">Punto de partida · {project.workflow_stage.replaceAll("_", " ")}</p><h3>Objetivo declarado</h3><p>{project.objective || "Sin definir"}</p><h3>Territorio y responsables</h3><p className="muted">{[project.territory, project.group_company, project.participants].filter(Boolean).join(" · ") || "Sin definir"}</p><h3>Brief</h3><p className="muted">{project.brief || "Sin contexto adicional"}</p></>}
